...
int main(int argc, char** argv) {
  ...
  try {
    ...
    Eigen::VectorXd x {{0.0, 1.0, 0.0}};
    Eigen::VectorXd y {{0.0, 0.0, 1.0}};
    Eigen::VectorXd z {{1.0, 0.0, 0.0}};
    ...
    double total_time = 0;
    //rot = Eigen::Matrix3d(initial_transform.rotation());
    Eigen::Quaterniond orientation_d(rot);
    ...
    std::function<franka::Torques(const franka::RobotState&, franka::Duration)>
      ...
      // get state variables
      total_time += time.toMSec();
      std::cout << total_time / 1000 << std::endl;
      ...
      std::cout << error.tail(3).norm() << std::endl;
      //std::cout << std::endl;
      return tau_d_array;  
    };
